...
export default class Leaf {
  ...
  search(mid: string): string | null {
    // 返回节点数据
    const [res]: any[] = this.elementList.filter(
  ...
  moveNode(mid: string, n: number, data: any): object[] {
    ...
    // 判断是否是将父元素移动到子元素下
    // 查询出所有父节点的mid
    const [node]: any[] = this.elementList.filter(
      (item: any) => item._mid === mid
    );
    ...
    if (n === 2) {
      ...
      const index = this.elementList.filter((item: any) => item._pid === mid)
        .length;
      if (!data._pid) {
        // 将根元素移动到某元素后面
        this.elementList.forEach((item: any) => {
  ...
  appendNode(mid: string, n: number, data: any, fb?: Function): Array<object> {
    ...
    if (n === 2) {
      ...
      const _index = this.elementList.filter((item: any) => item._pid === mid)
        .length;
      const _mid = data._mid || getRandom();
      this.activeMid = _mid;
      const newElement = fb ? fb({ _mid, ...data }) : { _mid, ...data };
  ...
  static data2tree(data: any[]) {
    ...
    function array2Tree(arr, pid) {
      ...
      for (var j = 0, max = res.length; j < max; j++) {
        var item = res[j];
        item.children = array2Tree(arrCopy, item._mid).sort(
